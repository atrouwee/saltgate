...
| **Vision3 250D** | `silbersalz-250d_v2-paired_33.cube` | **Beta** — 22 pairs, two donors, four rolls (thanks Sebastian and Lukas Walter) | held out a whole roll at a time: median ΔE2000 **7.8** — a *cross-donor* figure, where the earlier 3.7 was measured inside one photographer's own rolls. Give every frame its own exposure and both donors land at **2.0** and **2.4**, against a 2.2 ΔE floor the 8-bit delivery format imposes on its own: there is no measurable colour disagreement between them. What remains is per-roll density, which no LUT can know — nudge exposure to taste |
| Vision3 250D — previous | `silbersalz-250d_v0.1-statistical_33.cube` | **Proxy** — superseded | the pre-pairs stand-in, estimated from ~700 graded frames. Still selectable so a project begun on it can be finished on it; it is not the better LUT |
