...
def test_a_big_machine_gets_more_than_one_worker(monkeypatch):
    ...
    monkeypatch.setattr(ap, "available_gb", lambda: 28.0)
    monkeypatch.setattr(ap.os, "cpu_count", lambda: 12)
    assert ap.default_workers() == 4


def test_a_big_but_busy_machine_gets_fewer(monkeypatch):
    """Total RAM says how big the machine is, not how much is left: a 32 GB
    machine with 4 GB actually free once froze mid-roll on the old formula."""
    monkeypatch.setattr(ap, "ram_gb", lambda: 32.0)
    monkeypatch.setattr(ap, "available_gb", lambda: 4.0)
    monkeypatch.setattr(ap.os, "cpu_count", lambda: 12)
    assert ap.default_workers() == 2
